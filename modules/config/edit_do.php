...
//--------------------------------------------------------------------------------	
	...
	//echo $extra;
//-------------------------------------------------------------------------------------	
	$sql="Select * from config_variable where 1 $extra and school_id='".$_SESSION[ "current_school_id" ]."'";
	...
	if(numrows($res)>0){
		while($rec=dofetch($res)){
			...
			switch($type){
				...
				case "editor":
				case "textarea":
					$sql="Update config_variable set value='".slash($_POST[$type."_".$rec["id"]])."' where id='".$rec["id"]."' and school_id='".$_SESSION[ "current_school_id" ]."'";
					doquery($sql,$dblink);
				break;
				case "checkbox":
					$f_value="";
					...
					$sql="Update config_variable set default_values='".slash($f_value)."' where id='".$rec["id"]."' and school_id='".$_SESSION[ "current_school_id" ]."'";
					doquery($sql,$dblink);
				break;
				case "radio":
				case "combobox":
				...
				case "file":
					if(!empty($_FILES[$type."_".$rec["id"]]["tmp_name"])){
						...
						$sql="Update config_variable set value='".$img."' where id='".$rec["id"]."' and school_id='".$_SESSION[ "current_school_id" ]."'";
						doquery($sql,$dblink);
