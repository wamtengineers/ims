...
if(isset($_GET["id"]) && !empty($_GET["id"])){
    $students = doquery( "select a.*, b.class_section_id , concat( d.class_name, ' (', c.title, ')') as class_name, d.id as class_id, d.class_name as class from student a left join student_2_class b on a.id = b.student_id and academic_year_id = '".$year_id."' left join class_section c on b.class_section_id = c.id left join class d on c.class_id = d.id where a.id = '".slash( $_GET[ "id" ] )."'", $dblink );
}
else{
    if(isset($_SESSION["student_manage"]["list"]["class_section_id"])){
        ...
        header("Location: student_manage.php?err='".url_encode("Select Class First"));
        die;
    } 
}
if( isset( $_GET[ "examination_type_id" ] ) ) {
    $exam =  doquery( "select a.*, b.title, b.generate_marksheet from examination a left join examination_type b on a.examination_type_id = b.id where a.status = 1 and b.status = 1 and a.examination_type_id = '".slash($_GET["examination_type_id"])."' and academic_year_id = '".slash($year_id)."'", $dblink ); 
    if ( numrows($exam)>0) {
        $exam = dofetch($exam);
    }
    else{
	...
	if( is_array( $result ) ) {
		...
		<div class="page-break-after: always">
			<table class="main-header">
				<tr>
					<td class="logo"><img src="<?php echo $site_url?>/uploads/<?php echo $_SESSION[ "session_school_id" ];?>/config/<?php echo $school_logo?>" alt=""></td>
					<td>
			...
			<table border="0" width="100%">
				...
				<tr>
					...
					<td width="14%"><span class="student_meta"><?php echo !empty($student["birth_date"])?date_convert( $student["birth_date"] ):'';?></span> </td>
				...
				<tr>
					<td>Age:</td>
					<td><span class="student_meta"><?php if(!empty($student["birth_date"])){ show_age( $student["birth_date"], $exam[ "result_date" ]); }?></span> </td>
					<td>Average Age:</td>
			...
			<table class="marks">
				<tr>
					...
					?>
					<th colspan="<?php echo $exam[ "generate_marksheet" ]==0?'5':($exam[ "generate_marksheet" ]==1?13:8)?>"><?php echo unslash( $exam[ "title" ] )?></th>
					<?php
					if( $exam[ "generate_marksheet" ]==2 ){
						?>
				...
				<tr>
					...
					<th><?php echo $percent_grade[ "grade" ]?></th>
					<th><?php
						echo $fail_count>0?'FAIL':($fail_count==1?"PROMOTED":($result["rank"]<=3?ordinal( $result["rank"] ):'PASS'));
					?></th>
				</tr>
			</table>
			<?php
