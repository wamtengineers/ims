...
if(isset($_GET["action"]) && $_GET["action"]!=""){
	...
	if(empty($err)){
		if($bulk_action=="delete"){
			...
			header("Location: student_manage.php?tab=list&msg=".url_encode("Records Deleted."));
			die;
		}
		if($bulk_action=="statuson"){
			$i=0;
			while($i<count($id)){
				doquery("update student set status=1 where id='".$id[$i]."'",$dblink);
				$i++;
			}
			header("Location: student_manage.php?tab=list&msg=".url_encode("Records Status On."));
			die;
		}
		if($bulk_action=="statusof"){
			$i=0;
			while($i<count($id)){
				doquery("update student set status=0 where id='".$id[$i]."'",$dblink);
				$i++;
			}
			header("Location: student_manage.php?tab=list&msg=".url_encode("Records Status Off."));
			die;
		}
	}
	else{
		header("Location: student_manage.php?tab=list&err=".url_encode($err));
		die;					
	}
}
else{
