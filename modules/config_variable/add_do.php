...
if(isset($_POST["config_variable_manage"])){
	...
	if($err==""){
		$sql="INSERT INTO config_variable (school_id, config_type_id, title, notes, type, default_values, `key`, `value`) VALUES ('".$_SESSION["current_school_id"]."','".slash($config_type_id)."','".slash($title)."','".slash($notes)."','".slash($type)."','".slash($default_values)."','".slash($key)."','".slash($value)."')";
		doquery($sql,$dblink);
		$id=inserted_id();
