...
if(isset($_POST["admin_type_add"])){
	...
	if($err==""){
		$sql="INSERT INTO admin_type (school_id, title, can_add, can_edit, can_delete, can_read) VALUES ('".$_SESSION["current_school_id"]."', '".slash($title)."','".slash($can_add)."','".slash($can_edit)."','".slash($can_delete)."','".slash($can_read)."')";
		doquery($sql,$dblink);
