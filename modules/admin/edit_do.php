...
if(isset($_POST["admin_edit"])){
	...
	if($err==""){
		$sql="Update admin set ".($_SESSION[ "logged_in_admin" ][ "school_id" ] == 0?"`school_id`='".slash($school_id)."',":"")."`admin_type_id`='".slash($admin_type_id)."',`username`='".slash($username)."',`name`='".slash($name)."', `email`='".slash($email)."'".(!empty($password)? ", `password`='".md5($password)."'":"")." where id='".$id."'".($_SESSION[ "logged_in_admin" ][ "school_id" ] != 0?" and school_id='".$_SESSION[ "logged_in_admin" ][ "school_id" ]."'":"")."";
		doquery($sql,$dblink);
...
if(isset($_GET["id"]) && $_GET["id"]!=""){
	$rs=doquery("select * from admin where id='".slash($_GET["id"])."'".($_SESSION[ "logged_in_admin" ][ "school_id" ] != 0?" and school_id='".$_SESSION[ "logged_in_admin" ][ "school_id" ]."'":"")."",$dblink);
	if(numrows($rs)>0){
		$r=dofetch($rs);
