...
if(isset($_POST["menu_edit"])){
	...
	if($err==""){
		...
		doquery("delete from menu_2_school where menu_id='".$id."'", $dblink);
		if( isset( $school_ids ) && count( $school_ids ) > 0 ) {
			foreach($school_ids as $school_id){
				doquery( "insert into menu_2_school values('".$id."', '".$school_id."')", $dblink );
			}
		}
		...
		if( !isset($_POST["school_ids"]) )
			$_SESSION["menu_manage"]["edit"]["school_ids"] = array();
...
if(isset($_GET["id"]) && $_GET["id"]!=""){
	...
	if(numrows($rs)>0){
		...
		$school_ids = array();
		$rs =doquery("select school_id from menu_2_school where menu_id='".$id."'", $dblink);
		if( numrows( $rs ) > 0 ) {
			while( $r = dofetch( $rs ) ) {
