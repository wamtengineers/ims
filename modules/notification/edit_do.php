...
if(isset($_POST["notification_edit"])){
	...
	if($err==""){
		...
		doquery("delete from notification_2_department where notification_id='".$id."'", $dblink);
...
if(isset($_GET["id"]) && $_GET["id"]!=""){
	...
	if(numrows($rs)>0){
		...
		foreach($r as $key=>$value)
			...
			$department="select * from notification_2_department where notification_id='".$id."'";
			$rs2 = doquery( $department, $dblink );
			if( numrows( $rs2 ) > 0 ) {
				while( $r2 = dofetch( $rs2 ) ) {
					$department_ids[] = $r2[ "department_id" ];
				}
			}
