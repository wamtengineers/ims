	...
	if($r[ "target" ]==0){
		...
		if( numrows( $rs2 ) > 0 ) {
			while( $r2 = dofetch( $rs2 ) ) {
				...
				$sms_number = get_student_meta( $r2[ "id" ], "phone");
				sendsms_bulk(unslash($sms_number), $sms);
			}
		}
	}
	else{
		$check = doquery( "select department_id from notification_2_department where notification_id = '".$r[ "id" ]."'", $dblink );
		$check2 = doquery( "select employee_id from notification_2_employee where notification_id = '".$r[ "id" ]."'", $dblink );
		if( numrows( $check2 ) > 0 ) {
			$rs2 = doquery("select * from employee where id in (select employee_id from notification_2_employee where notification_id = '".$r[ "id" ]."')", $dblink);
		}
		else{
			$rs2 = doquery("select * from employee where status=1".(numrows($check)>0?" and department_id in (select department_id from notification_2_department where notification_id = '".$r[ "id" ]."')":""), $dblink);
		}
		if( numrows( $rs2 ) > 0 ) {
			while( $r2 = dofetch( $rs2 ) ) {
				$sms_number =  get_employee_meta( $r2[ "id" ], "mobile_number");
				$sms=str_replace(array(
					"%employee_name%",
					"%today%"
				), array(
