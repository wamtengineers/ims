...
$student_meta_fields = array( "nationality","birth_place","cnic_no","gender","religion","phone","corresponding_address","corresponding_phone","father_occupation","father_email","father_number","guardian","mother_name","mother_occupation","mother_email","mother_office_phone","gr_no","admission_level","group","board_name","domicile_district","notes","last_school_attended_to_date","last_school_attended_from_date","balance" , "houses_id");
function admin_logo(){
...
function sendsms($mobile_number, $text){
	...
	if( !isset( $_SESSION[ "sms_session" ] ) ) {
		...
		if( strpos( $response->data, 'Error' ) === false  ){
			$_SESSION[ "sms_session" ] = $response->data;
		}
	}
	if( isset( $_SESSION[ "sms_session" ] ) ) {
		...
		if( strpos( $response, 'Error 102' ) !== false ){
...
function get_total_active_students(){
	global $dblink;
	$rs=dofetch(doquery("select count(1) from student a inner join student_2_class b on a.id=b.student_id where a.status=1 and b.status=1", $dblink));
	return $rs["count(1)"];
}
function get_total_active_employees(){
	global $dblink;
	$rs=dofetch(doquery("select count(1) from employee where status=1", $dblink));
	return $rs["count(1)"];
}

function current_academic_year() {
	global $dblink;
	$rs=dofetch(doquery("select * from academic_year where is_current_year=1", $dblink));
	return $rs;
}
...
function get_chalan_arrears( $chalan ){
	...
	$rs = dofetch( doquery( "select sum( fees_amount ) as total from fees_chalan a inner join fees_chalan_details b on a.id = b.fees_chalan_id where academic_year_id = '".$chalan[ "academic_year_id" ]."' and month < '".$chalan[ "month" ]."' and student_2_class_id = '".$chalan[ "student_2_class_id" ]."'", $dblink ) );
	$total = $rs[ "total" ];
	...
	$rs = dofetch( doquery( "select sum( amount ) as total from fees_chalan a inner join fees_chalan_receiving b on a.id = b.fees_chalan_id where academic_year_id = '".$chalan[ "academic_year_id" ]."' and month < '".$chalan[ "month" ]."' and student_2_class_id = '".$chalan[ "student_2_class_id" ]."'", $dblink ) );
	$receiving = $rs[ "total" ];
...
function get_chalan_details( $chalan ) {
	...
	$rs = doquery( "select * from fees_chalan_details a inner join fees b on a.fees_id = b.id where fees_chalan_id	 = '".$chalan[ "id" ]."' order by b.sortorder", $dblink );
...
function generate_chalan( $student_id, $student_2_class_id, $academic_year_id, $month, $issue_date, $academic_year_start_ts, $due_date = "", $month_from = '' ){
	...
	doquery( "insert into fees_chalan( student_2_class_id, academic_year_id, month, issue_date, late_fees, due_date ) values( '".$student_2_class_id."', '".$academic_year_id."', '".$month."', '".$issue_date."', '".$late_fees."', '".$due_date."' )", $dblink );
